...
namespace FeiNuo.Core
{
    ...
    public class PageHelper
    {
        ...
        public static async Task<PageResult<TEntity>> FindPagedlist<TEntity>(IOrderedQueryable<TEntity> query, Pager pager) where TEntity : class, new()
        {
            ...
            {
                var lstData = await query.AsNoTracking().ToListAsync();
                return new PageResult<TEntity>(lstData, lstData.Count);
            }
            // 查询分页数据
            var skipSize = Math.Max(pager.PageNo - 1, 0) * pager.PageSize;
            var pageData = await query.AsNoTracking().Skip(skipSize).Take(pager.PageSize).ToListAsync();
            // 查询总数量
        ...
        /// 添加排序条件，分页前必须排序，
        /// </summary>
        public static IOrderedQueryable<TEntity> ApplySort<TEntity>(IQueryable<TEntity> query, Pager pager, Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>> defaultOrder) where TEntity : class, new()
        {
